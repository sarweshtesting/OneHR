...
/** Same rendering approach as ReportExportService, but for team-wide exports
 * (one row per employee per day) which need an Employee column. */
@Component
public class TeamReportExportService {

    private static final String[] HEADERS = {"Date", "Employee", "Day", "Mode", "Clock In", "Clock Out", "Hours Worked", "Status"};
    private static final DateTimeFormatter TIME_FMT = DateTimeFormatter.ofPattern("HH:mm").withZone(ZoneOffset.UTC);

    public byte[] toCsv(List<TeamReportRow> rows) {
    ...
    public byte[] toXlsx(List<TeamReportRow> rows, String sheetName) {
    ...
    public byte[] toPdf(List<TeamReportRow> rows, String title) {
        try (PDDocument document = new PDDocument(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            float margin = 40;
            float rowHeight = 16;
            float[] colWidths = {55, 90, 45, 45, 50, 50, 60, 55};
    ...
    private String[] cells(TeamReportRow row) {
        return new String[] {
                row.date().toString(),
    ...
    private float writeRow(PDPageContentStream content, String[] cells, float[] colWidths, float margin, float y,
        ...
        for (int i = 0; i < cells.length; i++) {
            ...
            content.showText(truncate(cells[i], 22));
            content.endText();
            x += colWidths[i];
        }
        return y - rowHeight;
